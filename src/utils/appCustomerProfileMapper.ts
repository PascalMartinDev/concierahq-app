...
export const appCustomerProfileMapper = (
  ...
  if (customerProfile.booking) {
    ...
    if (customerProfile.booking.booking_date_time) {
      appCustomer.bookings.bookingDateTime =
        customerProfile.booking.booking_date_time;
    }
    // Set Booking Duration
    if (customerProfile.booking.booking_duration) {
      appCustomer.bookings.bookingDuration =
        customerProfile.booking.booking_duration;
    }
    // Set Booking Is Vip
    if (customerProfile.booking.booking_is_vip) {
      appCustomer.bookings.bookingIsVip =
        customerProfile.booking.booking_is_vip;
    }
    // Set Booking Notes
    ...
    if (customerProfile.booking.booking_no_people) {
      appCustomer.bookings.bookingNoPeople =
        customerProfile.booking.booking_no_people;
    }
    // Set Booking Service Name
    if (customerProfile.booking.booking_service_name) {
      appCustomer.bookings.bookingServiceName =
        customerProfile.booking.booking_service_name;
    }
    // Set Booking Status
    if (customerProfile.booking.booking_status) {
      appCustomer.bookings.bookingStatus =
        customerProfile.booking.booking_status;
    }
    // Set Booking table Name Number
    if (customerProfile.booking.booking_table_name_number) {
      appCustomer.bookings.bookingTableNameNumber =
        customerProfile.booking.booking_table_name_number;
    }
    // Set Booking Table Section
    if (customerProfile.booking.booking_table_section) {
      appCustomer.bookings.bookingTableSection =
        customerProfile.booking.booking_table_section;
    }
    // Set Booking Tags List
    if (customerProfile.booking.booking_tags_list) {
      appCustomer.bookings.bookingTagList =
        customerProfile.booking.booking_tags_list;
    }
  }
  ...
  if (customerProfile.ecommerce) {
    ...
    if (customerProfile.ecommerce.e_commerce_customer_id) {
      appCustomer.eCommerce.eCommerceCustomerId =
        customerProfile.ecommerce.e_commerce_customer_id;
    }
    // Set Subscription:
    if (customerProfile.ecommerce.subscription) {
      ...
      if (customerProfile.ecommerce.subscription.next_subscription_date) {
        appCustomer.eCommerce.subscription.subscriptionNextDate =
          customerProfile.ecommerce.subscription.next_subscription_date;
      }
      ...
      if (customerProfile.ecommerce.subscription.subscription_cancelled_date) {
        appCustomer.eCommerce.subscription.subscriptionCancelledDate =
          customerProfile.ecommerce.subscription.subscription_cancelled_date;
      }
      ...
      if (customerProfile.ecommerce.subscription.subscription_joined_date) {
        appCustomer.eCommerce.subscription.subscriptionJoinedDate =
          customerProfile.ecommerce.subscription.subscription_joined_date;
      }
      // Set Subscription Status:
      if (customerProfile.ecommerce.subscription.subscription_status) {
        appCustomer.eCommerce.subscription.subscriptionStatus =
          customerProfile.ecommerce.subscription.subscription_status;
      }
      // Set Subscription Level:
      if (customerProfile.ecommerce.subscription.subscription_level) {
        appCustomer.eCommerce.subscription.subscriptionLevel =
          customerProfile.ecommerce.subscription.subscription_level;
      }
    }
  }
  ...
  if (
    import.meta.env.VITE_INTEGRATION_KLAVIYO == 'true' ||
    import.meta.env.VITE_INTEGRATION_KEAP == 'true'
  ) {
